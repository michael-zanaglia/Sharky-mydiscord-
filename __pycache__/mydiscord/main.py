...
class Connexion() :
    ...
    def ChangeImg(self) :
        try :
            ...
            print("Chemin du fichier :", path)  # Vérification du chemin du fichier
            buffered = BytesIO()
            ext=pathlib.Path(path).suffix
            ext=ext.replace('.','').upper()
            if ext=='JPG':
                ext='JPEG'
            path = os.fsencode(path).decode("utf-8")
            print("Chemin du fichier après conversion :", path)  # Vérification du chemin du fichier après conversion
        ...
        if event.type == pygame.MOUSEBUTTONDOWN :
            ...
            if active :
                if p.yes.collidepoint(posx -((p.L-500)//2), posy - ((p.l-125)//2)) :
                    ...
                    co.cursor.execute("delete from run where pseudo = '{}'".format(p.pseudo))
                    co.conn.commit()
                    pygame.quit()
                    sys.exit()
